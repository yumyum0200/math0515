...
# ---------------------------------------------------------
    ...
    # --- [2, 3, 4, 5페이지] 3:7 레이아웃 적용 ---
    else:
        col_l, col_r = st.columns([3, 7])
        with col_l:
            st.markdown(f'<div class="side-problem"><b>[문제]</b><br>{p_text}</div>', unsafe_allow_html=True)
            # 4, 5단계에서는 시각적 도움을 위해 식을 추가로 보여줌
        ...
        with col_r:
            ...
            if st.session_state.page == 2:
                ...
                if st.button("💡 힌트"): st.warning("문제를 다시 읽어보세요! 구하라고 하는 것이 명시되어 있습니다.")
                target = "전체 유기견의 수" if st.session_state.mission == 1 else ("처음 종이의 한 변의 길이" if st.session_state.mission == 2 else "x의 값")
                ans = st.selectbox("구하려고 하는 것은?", ["선택하세요", "상자의 높이", "전체 비스킷 수", target, "감귤의 무게"])
            ...
            # --- [Step 2] ---
                ...
                if st.session_state.mission == 1:
                    st.selectbox("2-(1) 문제 이해: 전체 유기견 40마리일 때 한 마리당 비스킷 수는?", ["선택하세요", "13개", "27개", "67개"])
                    st.selectbox("2-(2) 단서 찾기: 유기견 x마리 일 때, 한 마리당 비스킷 수를 x로 나타내면?", ["선택하세요", "x + 27", "x - 27", "27 - x"])
                    st.selectbox("2-(3) 식 세우기:", ["선택하세요", "x + (x - 27) = 90", "x × (x - 27) = 90", "x - 27 = 90"])
                elif st.session_state.mission == 2:
                    st.selectbox("2-(0) 기초 상식: 부피 공식은?", ["선택하세요", "(가로)×(세로)×(높이)", "(가로)+(세로)+(높이)"])
                    st.selectbox("2-(1) 문제 이해: 처음 한 변 20cm일 때 상자의 한 변은?", ["선택하세요", "10cm", "15cm", "5cm"])
                    st.selectbox("2-(2) 단서 찾기: 처음 한 변 xcm일 때 상자의 한 변은?", ["선택하세요", "(x - 10)cm", "(x - 5)cm"])
                    st.selectbox("2-(3) 식 세우기:", ["선택하세요", "(x-10) × (x-10) × 5 = 4500", "(x-5) × (x-5) × 5 = 4500"])
                else:
                    st.selectbox("2-(1) 문제 이해: 전체 감귤의 양은?", ["선택하세요", "2000 × 4200", "2000 + 4200"])
                    st.selectbox("2-(2) 단서 찾기: 한 상자에 들어가는 감귤 양은?", ["선택하세요", "2000 + x", "2000 - x"])
                    st.selectbox("2-(3) 단서 찾기: 만들 수 있는 상자의 개수는?", ["선택하세요", "4200 - 2x", "4200 + 2x"])
                    st.selectbox("2-(4) 식 세우기:", ["선택하세요", "(2000+x)×(4200-2x)=2000×4200", "(2000+x)+(4200-2x)=8,400,000"])
            ...
            # --- [Step 3] ---
                ...
                if st.session_state.mission == 1:
                    ...
                    st.selectbox("3-(3) 왜 x = -3은 답이 될 수 없을까요?", ["선택하세요", "유기견 수는 자연수여야 하므로", "비스킷이 부족해서"])
                elif st.session_state.mission == 2:
                    st.selectbox("3-(1) 제곱근을 이용한 풀이 중 바른 식은?", ["선택하세요", "x - 10 = ±30", "x - 10 = ±900"])
                    st.selectbox("3-(2) 위 방정식의 해는?", ["선택하세요", "x = -20 또는 x = 40", "x = 20 또는 x = -40"])
                    st.selectbox("3-(3) 왜 x = -20은 답이 될 수 없을까요?", ["선택하세요", "길이는 양수여야 하므로", "부피가 너무 커서"])
                else:
                    st.selectbox("3-(1) 식을 바르게 정리한 것은?", ["선택하세요", "x^2 - 100x = 0", "x^2 + 100x = 0"])
                    st.selectbox("3-(2) 위 식을 바르게 인수분해 한 것은?", ["선택하세요", "x(x - 100) = 0", "x(x + 100) = 0"])
                    st.selectbox("3-(3) 인수분해 결과를 보고 해를 고르면?", ["선택하세요", "x = 0 또는 x = 100", "x = 100"])

            # --- [Step 4] ---
            elif st.session_state.page == 5:
                st.header("Step 4. 확인하기")
                if st.session_state.mission == 1:
                    v1 = st.number_input("유기견의 수 (x)", min_value=0, key="v1")
                    v2 = st.number_input("한 마리당 비스킷 수 (x - 27)", min_value=0, key="v2")
                    if st.button("결과 확인") and v1 * v2 == 90: st.balloons(); st.success("문제를 바르게 해결했습니다!")
                elif st.session_state.mission == 2:
                    v1 = st.number_input("처음 종이 한 변 (x)", min_value=0, key="v1")
                    v2 = st.number_input("상자 밑면 한 변 (x - 10)", min_value=0, key="v2")
                    if st.button("결과 확인") and v2 * v2 * 5 == 4500: st.balloons(); st.success("문제를 바르게 해결했습니다!")
                else:
                    v1 = st.number_input("더 넣는 귤의 양 (x)", min_value=0, key="v1")
                    v2 = st.number_input("상자의 개수 (4200 - 2x)", min_value=0, key="v2")
                    if st.button("결과 확인") and (2000 + v1) * v2 == 8400000: st.balloons(); st.success("문제를 바르게 해결했습니다!")